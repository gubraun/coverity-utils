...
class CoverityDefectService(Service):
    ...
    def SeekNthMatch(self, thisEventHistory, nthEvent, attributeName, attributeValue):
        resFound = False
        ...
        for thisDefectStateDO in thisEventHistory:
            ...
            # if there was a suitable named attribute
            if reqEventFound:
                numMatch += 1
    ...
    def getEventForAttributeChange(self, thisStreamDefectDO, nthTerm, attributeName, attributeValue=None):
        ...
        if nthTerm >= 0:
            found, thisDefectStateDO, value = self.SeekNthMatch(thisStreamDefectDO.history, nthTerm, attributeName, attributeValue)
        else:
            found, thisDefectStateDO, value = self.SeekNthMatch(reversed(thisStreamDefectDO.history), abs(int(nthTerm)), attributeName, attributeValue)
...
#            snapshotGrammar = covDS.snapshotEarlier
    ...
    for thisMergedDefectDO in allMergedDefectsPageDO.mergedDefects:
        ...
        # grab the last report-tagged Comment that was added
        nth = -1

        # look for the last triage event with this "Event" characteristic defined as "Report" (might be after the dismissal occurred)
        foundReportEvent, eventDefectStateDO, eventValue = covDS.getEventForAttributeChange(streamDefectDO, nth, "Event", "Report")

        if foundReportEvent:
            RepDebug(1, "Report event occurred on %s" % str(eventDefectStateDO.dateCreated))
            # now get the value of an associated attribute at the same time as this event
            foundAttr, covReportComment = covDS.getEventAttributeValue(eventDefectStateDO, "Comment")
            if foundAttr:
                RepDebug(1, "Report-Comment: [%s]" % covReportComment)
            else:
                RepDebug(2, "No comment defined for this report event??")

        # if no custom 'Event' attribute or no 'report' 'Event"
        else:
            # grab either the last comment or all comments @CHANGE as required
#            nth = -1
            nth = 0

            # grab the last (nth = -1) filled "Comment"
            if nth == -1:
                foundComment, eventDefectStateDO, covReportComment = covDS.getEventForAttributeChange(streamDefectDO, nth, "Comment")
                if foundComment:
                    RepDebug(1, "Last-Comment: [%s]" % covReportComment)
                else:
                    RepDebug(2, "No Comments found??")
            else:
                # get all (nth = 0) Comments in chronological order (joined together)
                foundComment, eventDefectStateDO, covReportComment = covDS.getEventForAttributeChange(streamDefectDO, 0, "Comment")
                if foundComment:
                    RepDebug(1, "All-Comments: [%s]" % covReportComment)
                else:
                    RepDebug(2, "No Comments found??")
